...
######################################################################
# Single-stage build: all kernel modules and drivers are sourced
# from the pre-compiled packages in the DOCA repository.
# Use bluefield-ocp.Containerfile when on-the-fly OFED/SoC compilation
...
RUN \
  ...
  #
  # Install kernel modules and SoC drivers from pre-compiled DOCA repo packages.
  # doca-runtime-kernel pulls in all OFED KMPs and SoC driver KMPs as dependencies.
  dnf install -y --setopt=install_weak_deps=False \
  doca-runtime-kernel \
  doca-runtime-user; \
  #
  # Install doca-runtime meta package without its dependencies
  cd /tmp; \
  dnf download doca-runtime && \
  rpm -ivh --nodeps \
